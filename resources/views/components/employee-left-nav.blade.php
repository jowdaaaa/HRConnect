<aside class="w-64 bg-white shadow-lg flex flex-col h-screen sticky top-0">
    ...
    @php
        $userNavUsername = session('hr_user_username', 'Employee');
        $userNavRole = session('hr_user_role', 'Employee');
    @endphp
    <div class="p-6 border-b border-gray-200">
        <div class="flex flex-col items-center">
            ...
            <span class="text-xs text-gray-500 mt-1">
                {{ ucfirst(strtolower($userNavRole)) }}
            </span>
        </div>
    </div>
    ...
    <!-- Navigation Menu -->
    @php
        $isDashboard = request()->is('employee-dashboard');
        $isReports = false;
        $isSettings = false;
    @endphp
    <nav class="flex-1 px-4 py-6 overflow-y-auto">
        ...
        <div class="space-y-2">
            <a href="/employee-dashboard" class="flex items-center px-4 py-3 text-sm rounded-lg transition-all {{ $isDashboard ? 'font-semibold text-[#009944] bg-[#009944]/10' : 'font-medium text-gray-700 hover:bg-gray-100 hover:text-[#009944]' }}">
    ...
    <div class="p-4 border-t border-gray-200">
        ...
        <button type="button" class="logout-confirm-btn w-full flex items-center px-4 py-3 text-sm font-medium text-red-600 rounded-lg transition-all hover:bg-red-50 hover:text-red-700">
